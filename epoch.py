...
def train_epoch(
    ...
    """Single train epoch pass. At the end of the epoch, updates the.
    lists loss_train, acc_train and topk_acc_train.
    """
    model.train()
    ...
    for batch_idx, (batch_x_train, batch_y_train) in enumerate(
        ...
        with torch.no_grad():
            n_correct_train += torch.sum(
                torch.eq(batch_y_train, torch.argmax(
                    batch_output_train, dim=-1))
            ).item()
            for k in list_k:
    ...
    # At the end of epoch compute average of statistics over batches and store
    # them
    with torch.no_grad():
...
def val_epoch(
    ...
    At the end of the epoch, updates the lists loss_val, acc_val,
    topk_acc_val and avgk_acc_val.
    """
    ...
    with torch.no_grad():
        ...
        loss_epoch_val = 0
        n_correct_val = 0
        n_correct_topk_val, n_correct_avgk_val = defaultdict(
            int), defaultdict(int)
        topk_acc_epoch_val, avgk_acc_epoch_val = {}, {}
        # Store avg-k threshold for every k in list_k
        lmbda_val = {}
        # Store class accuracy, and top-k and average-k class accuracy
        # for every k in list_k
        class_acc_dict = {}
        ...
        # Store estimated probas and labels of the whole validation set to
        # compute lambda
        list_val_proba = []
        list_val_labels = []
        for batch_idx, (batch_x_val, batch_y_val) in enumerate(
            ...
            for k in list_k:
                ...
                update_correct_per_class_topk(
                    batch_proba, batch_y_val,
                    class_acc_dict["class_topk_acc"][k], k,
                )
        ...
        for k in list_k:
            # Computes threshold for every k and count nb of correctly
            classifier examples in the avg-k sense (globally and for each class)
            lmbda_val[k] = 0.5 * (
...
def test_epoch(
    ...
    with torch.no_grad():
        ...
        n_correct_topk_test, n_correct_avgk_test = defaultdict(
            int), defaultdict(int)
